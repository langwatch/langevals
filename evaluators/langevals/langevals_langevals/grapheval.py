...
class GraphEvalEvaluator(
    ...
    def evaluate(self, entry: GraphEvalEntry) -> SingleEvaluationResult:
        details = None
        try:
            knowledge_graph = self._construct_knowledge_graph(entry.output)
        except Exception as e:
            print("Caught an exception: ", e)

        passed = self._compare_knowledge_graph_with_contexts(
            knowledge_graph=knowledge_graph, contexts=entry.contexts
        )

        return GraphEvalResult(passed=passed, details=details)

    def _construct_knowledge_graph(self, output: str) -> list[str]:
        """Creates and returns a knowledge graph from the output of the llm

        Args:
            output (str): Output generated by the llm that is being evaluated on hallucinations
...
Important Tips
        ...
        print(arguments.get("triples", "No triples were found"))
        return arguments.get("triples", "No triples were found")
    ...
    def _compare_knowledge_graph_with_contexts(
        ...
        Args:
            knowledge_graph (list[str]): _description_

        Returns:
            bool: _description_
        """
        knowledge_graph_is_factual = True
        ...
        print(
            arguments.get(
                "result",
                "Faithfulness could not be evaluated",
            )
        )
        knowledge_graph_is_factual = arguments.get(
            "result",
            "Faithfulness could not be evaluated",
        )

        return knowledge_graph_is_factual
